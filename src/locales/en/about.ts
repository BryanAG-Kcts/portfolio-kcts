export const about = {
  title: 'Get to know me better',
  paragraphs: [
    'My full name is Bryan David Álvarez Galvis, systems engineering student, lover of web development, programming and cacti. Tired of spending my time looking for and using third-party tools and software, I now create utilities for myself as well as for everyone.',
    'I am a leader in my own university project, together with my team, we came in second place in the UFPSO junior category classroom project invention fair.',
    "Our project is Sittingn't, and it will continue to evolve until it reaches completion... Like all of us one day. Thank you very much for your time, I hope you have been interested in my work and my experience."
  ]
}

export const softSkill = {
  title: 'Soft skills',
  paragraph: "Skills beyond programming. It's not just about programming well, it's about being a good programmer.",
  softSkills: [
    'Attention to detail',
    'Proactivity',
    'Adaptability',
    'Problem solving',
    'Leadership',
    'Responsibility',
    'Creativity',
    'Continuous improvement',
    'Initiative'
  ]
}
